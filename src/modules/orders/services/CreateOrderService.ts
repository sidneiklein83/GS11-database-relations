...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (findProducts.length !== products.length) {
      throw new AppError('Existem produtos inexistentes! Verifique.');
    }
    //
    ...
    const updatedProducts = findProducts.map(findProduct => {
      ...
      if (orderProduct) {
        if (findProduct.quantity < orderProduct.quantity) {
          throw new AppError(
            `
              O Produto ${findProduct.name} não possui estoque suficiente: ${findProduct.quantity}\n
              Quantidade Solicitada: ${orderProduct.quantity}
            `,
          );
        }
